...
- **FR-1** The environment MUST provide confined sessions for `claude-code`, `opencode` and `pi`. All three are required, and `claude-code` is the reference case — not because it is the easiest, but because it is the agent whose configuration root is least certain to relocate, so taking it first front-loads the risk rather than deferring it. It was the reference case for a second reason once, that the other two would obtain their credential from it; `M7b` measured that they do not, so that reason and the sequencing that followed from it are both withdrawn.
...
- **FR-7** Authenticating once on a machine MUST serve every project on that machine, and every agent shipped. No agent logs in on another's behalf and none reads another's credential store: each declares the same service name and the supervisor mints each session an independent substitute, so the login belongs to the machine rather than to any agent. How is the plan's business, but FR-6 and FR-3 together already exclude the obvious shortcut: letting the other agents read the authenticating agent's credential store would put a credential that works outside the boundary inside it, and a grant for that store would rest on convenience rather than on the tool being structurally impossible to redirect.
