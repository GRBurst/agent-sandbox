...
| `check_r5` | Make the wrapper resolve `--profile` by name, or drop the flag so `NONO_PROFILE` decides | the path the checkout asked for is read from inside the session. Both forms were measured reading a canary out, so either bites | [ ] |
...
- **nono's remaining configuration surfaces** (R5's neighbourhood). `M5e` measured profiles, `NONO_PROFILE`, `NONO_ALLOW` and `--extends`. Three surfaces named by the binary are still unmeasured and so uncovered by any check: `$XDG_CONFIG_HOME/nono/config.toml`, whose keys are unknown and which the blanket `XDG_CONFIG_HOME` puts inside the project; a project-level `trust-policy.json`, which nono looks for in the current directory; and `--bypass-protection <PATH>`, documented as overriding a deny rule and therefore the one flag that might reach what `M5a` found the required deny groups do not actually protect. Measuring them is owed before the M5 group closes, and until then R5's assertion covers the profile channel only.
