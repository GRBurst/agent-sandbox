...
| `flake.nix`, `.envrc`, `.gitignore` | What is being rewritten; the three bootstrap variables must resolve to the same value in the first two |
...
| **C2** | `XDG_DATA_HOME="$PWD/.local/share"` is a second blanket, exported twice over — from `.envrc` as a bootstrap variable and from the `shellHook` — where **P1** allows `XDG_CACHE_HOME` alone | nix itself is the tool that needs it, and it needs it *before* the flake is evaluated, which is the one point at which no flake-declared variable can help. `M4b` watched `nix eval` on the `llm-agents.nix` input die with `opening file '…/.local/share/nix/trusted-settings.json': Permission denied` in a sandbox that denies `$HOME`: nix records the flake configuration a user has accepted there, and it writes before it will evaluate any flake declaring `nixConfig`. Ours does, and so does that input | There is no narrower variable. Nix's own knobs (`NIX_CONFIG`, `NIX_USER_CONF_FILES`) name where nix *reads* settings, not where it records an accepted `nixConfig`, and that record's path is derived from `XDG_DATA_HOME` with no override of its own. Declaring it in the `shellHook` alone is not an option either, since the failure happens while the hook is still being computed |

**What C1 costs, stated rather than discounted.** The blanket applies to every program run from the devShell, not only to a confined session, so an unconfined tool started from the project shell reads project configuration instead of the developer's own. That surfaced first as an unrelated GUI editor depositing its caches and local storage into `.config/`, which is why `.gitignore` now denies that directory by default. It is also why the developer's own agent, run unconfined from inside the checkout, loses its host configuration — a consequence of the *scope* of this variable, and not the confinement property that `FR-21` describes.

**What C2 costs.** It splits direnv's own trust store in two: the record of which `.envrc` a human has allowed lives under `XDG_DATA_HOME`, so a `direnv allow` granted from outside the project is invisible from inside it and has to be granted again. Every other consumer of the blanket is a tool writing data it can regenerate, which is why the cost is annoyance rather than loss.
...
- <a id="d5"></a>**D5 — The pre-flight is functional, not introspective.** It asserts *enforcement* by observing a denial, rather than probing kernel interfaces (`/sys/kernel/security/lsm`, Landlock ABI, cgroup v2). Introspection lost because the probe list is bubblewrap-shaped: nono uses Landlock and needs neither user namespaces nor cgroups v2, so a passing probe would prove the wrong thing, and a functional probe cannot pass for the wrong reason. Cost: two extra `nono` launches per agent start. Accepted unmeasured; if it hurts, cache per boot under `$XDG_RUNTIME_DIR`, which is a later change.
  **"A functional probe cannot pass for the wrong reason" was false as first sketched, and `M4a` corrected it.** The sketch put the canary at `$HOME/.agent-sandbox-preflight.$$` and read a failed write as proof of confinement. A write can fail because it was denied, or because it was never going to succeed — and on a host where `$HOME` is not writable by the user the two are indistinguishable from the outside. This repository's own development sandbox is such a host, which is how the defect was found, but it is not a property of it: the guard would report enforcement on any such machine having observed nothing. So the pre-flight now carries its own positive control, which is [D9](#d9)'s rule applied to shipped code rather than to a check. It **first writes the canary unconfined and requires that to succeed**, and only then reads the confined write's failure as a denial. Where no such path exists the pre-flight refuses with `77` rather than guessing, because "enforcement cannot be demonstrated" and "enforcement is absent" get the same fail-closed answer.
  The location is therefore chosen by observation rather than fixed: a path outside the project that the pre-flight has just proven it can reach, preferring `$XDG_RUNTIME_DIR` over `$HOME` because it is writable on a host whose home is not and is granted by no group. `M4a` measured both here — `$HOME` refuses the unconfined write, `$XDG_RUNTIME_DIR` takes it and a confined child writing there gets exit 1 with no file.
  **`M9e` corrected it again, and this is the shape that shipped.** The canary is gone: macOS has nowhere outside the project to write one, so the positive control was refusing on a platform that enforces perfectly well. What the probe observes now is a **read** of the candidate directory, and a read demonstrates enforcement exactly as well as a write while needing nothing writable anywhere. The two candidates became a loop rather than a choice, since a candidate can now be disqualified for reasons that say nothing about the host — inside the project, which the session is granted, or unreadable before confinement is applied — and the pre-flight moves on rather than concluding. Only an exhausted list refuses, and it refuses carrying every candidate's verdict, so the message names what was tried instead of asserting a negative.
...
- <a id="d14"></a>**D14 — The other agents take their credential from the mediated session, not from the first agent's store.** FR-7 spans agents as well as projects, and there were two ways to reach that. The prior art shares credentials by plugin: one agent shells out to another, or reads its credential file directly, which needs that credential directory granted read-write inside the boundary. This environment instead lets the mechanism capture the token flow into its supervisor-side store and exposes it to every process in the session as an environment variable plus a mediated base URL.
  ...
  **Which state is machine-scoped, which is project-scoped, and why that is not FR-4 being bent.** FR-7 wants one login per machine and FR-4 wants agent state inside the project, and the two would collide if the login were state an agent holds. It is not. The division is:
  ...
  | Project | everything else an agent writes — configuration, history, caches, memory, the VCS identity | under `$WORKDIR`, which is the property `check_confinement_validates` asserts over the whole table |
...
```text
.
├── flake.nix                        modified  description, 2 systems, devShell, packages, registry + table
├── flake.lock                       modified  nono + agent packages pinned
├── .envrc                           comment only the three bootstrap variables resolve alike
...
├── draft2.md                        deleted
├── lib/
│   ├── agents.nix                   new       the three agents: package, groups, credentials, state, skills
│   ├── leak-registry.nix            new       FR-3, the single file, expected empty
│   ├── confinement.nix              new       mkConfinement    :: agent -> store path
│   ├── confined-agent.nix           new       mkEntryPoint     :: name -> writeShellApplication
│   └── preflight.sh                 new       FR-11, sourced by the wrapper and by check_r6
├── scripts/
...
└── specs/001-agent-sandbox/
    ├── spec.md                      modified  the D2 and D14 corrections, made in place
    ├── plan.md                      new
    ├── tasks.md                     new
...
```nix
# mkConfinement ∷ { agent, name, registry, substrate, substrateVars } → derivation (a JSON file)
...
# The substrate arrives as a built derivation rather than a list, because D18
# reads its store paths at build time and never during evaluation.
{ lib, pkgs, agents, registry, substrate, substrateVars }:
name:
let
...
{ lib, pkgs, agentPkgs, agents, confinement }:
name:
let
...
pkgs.writeShellApplication {
  name = binary;
  # nono runs the session; the other four are the wrapper's own work — the
  # commit identity it copies (D11), the skill pointing it writes (D17), and
  # the manifests those two need.
  runtimeInputs = with pkgs; [ agentPkgs.nono git jq coreutils findutils ];
  text = ''
    ...
    preflight_or_die
    # D11 / FR-23, then D17 / FR-25 — inline, in this order. Both write into the
    # project create-if-absent, so entering twice changes nothing, and both are
    # the supervisor's work rather than the session's. The second collects one
    # --read per declared surface into agent_sandbox_reads.
    …
    exec nono run \
...
The positive control is the thing the first draft lacked, and [D5](#d5) records why it is not optional. **The sketch below is the shape `M9e` replaced, kept because the plants in the table below were run against it; the file is the truth.** What changed is the observable: the probe reads a directory instead of writing a canary into it, because macOS has nowhere outside the project to put a canary and a read demonstrates enforcement exactly as well as a write. The candidates are still `$XDG_RUNTIME_DIR` then `$HOME`, preferred in that order because the first is writable on a host whose home is not and is granted by no group, but each is now tried in turn — skipped when it lies inside the project or is unreadable to begin with — rather than one being chosen up front. A candidate that survives to be probed answers in one of three ways, so an unrefused read and an unrefused write are distinguished from each other and both from a probe that never ran; `die` gained a third argument, nono's own stderr, so the refusal quotes the mechanism rather than paraphrasing it.
...
# plus the derived-property checks, which carry no scenario id and are
# enumerated once, in the coverage table below.
```
...
- **Files touched**: `flake.nix`, `flake.lock`, `.gitignore`, new `lib/` (5 files — four `.nix` and `preflight.sh`), new `scripts/` (5 files), new `.github/workflows/verify.yml`, new `README.md`, `docs/HANDBOOK.md`, `docs/CONSTITUTION.md`, `AGENTS.md` (one sentence). Deleted: `devenv.nix`, `devenv.yaml`, `ai.nix`, `draft1.md`, `draft2.md`.
...
- **Tools added to the environment**: `nono` (the mechanism), `shellcheck` + `shfmt` (AGENTS.md names them; added to the devShell in `M3a`, having resolved only from a user profile until then), `jq` (checks parse JSON; P9 requires generated JSON be validated rather than eyeballed). The formatters and linters AGENTS.md §4's table names arrived with them, one per file kind — `nixfmt`, `mdformat`, `yamlfmt`, `yamllint`, `yq-go` — on the same rule: a tool that resolves only from a user profile is not available. `git` and `strace` are there because checks run them, `bash` because the scripts are bash and the interactive shell is zsh, and `just` and `pinact` are run by the `justfile`, which names §4's static stages so they are typed once rather than remembered.
...
| 2 | `nix flake check` | devShell and package evaluation on both systems. The flake declares no `checks` output, so this asserts evaluation only and none of the suite |
...
| 6 | `scripts/validate.sh` | The four behavioural layers, and stage 3 within them — `check_confinement_validates` validates every generated description against the schema. It runs none of the others |

Run stages 1–2 after every Nix edit, 4 after every shell edit, 6 before every commit.

**Stages 1, 2, 4 and 5 are a human's memory, and that is a gap rather than a design.** Only stage 3 ended up inside the entry point, and CI runs the entry point and nothing else, so nothing anywhere fails a run for an unformatted file or an unlinted script. Wiring them in is a change to the entry point's contract — it would start refusing on a formatting difference — so it is left for a follow-up rather than smuggled in at close-out. Listed in the coverage gaps below.
...
| Journey 2.1 | `check_confinement_validates` — that every relocated root is *under* the working directory, as a property over the agent table, and that the session redirects every `XDG_*` root the devShell hook exports, plus `XDG_STATE_HOME` and `TMPDIR` by name. Here rather than at the integration layer because `check_j2_1` reads its root set out of the description, so a root removed from both sides of that comparison is invisible to it. The two named roots are the two the derived mirror cannot reach: `XDG_STATE_HOME` because the devShell does not export it at all, and `TMPDIR` because the name is not an XDG one — and `TMPDIR` is also the only root whose absence is silent, since every XDG fallback lands under `HOME` and is denied while `/tmp` is granted. "Over the agent table" was drift until [`M8c`](research.md#m8c--opencode-needs-no-variable-of-its-own-and-takes-its-credential-from-the-environment): the check named one agent, so a second would have joined the table unasserted. It now loops the table and names the agent in every failure | component |
...
| Journey 2.1 | `check_opencode` — the same journey for the second agent, through the real entry point: every root it reports under the project, its credential arriving from the environment rather than from a store, the `$HOME` before-and-after diff, and the skill-surface file still valid JSON when the session ends. Named outside the pattern `check_sc3` scans, deliberately, because Journey 2.1 already has `check_j2_1` and a second check claiming the same id would break the bijection rather than strengthen it. **Control**: the project is compared before and after, so a session that wrote nothing anywhere cannot satisfy "nothing landed in `$HOME`" | integration |
| Journey 2.1 | `check_pi` — the third agent, asserted on the payload of its own credential report rather than on an exit status, because a lost relocation and a missing credential are both non-zero and only the payload distinguishes them. Second arm: the relocated root carries no installed package tree, which is FR-22's absence-of-a-fetch | integration |
| SC-2 | `check_registry` — registry invariants | unit |
| The suite's instruments | `check_trailer_parse` — the two parsers that read nono's stderr trailer, against recorded output from both platforms: a darwin session that refused nothing, a darwin session with denials, and a Linux session that prints no trailer at all. Here because the platforms differ in what silence means — on darwin it is a sentence, on Linux it is nothing — and a parser that conflated them would report every Linux session as clean | unit |
...
| `check_confinement_validates` | Delete `TMPDIR` from `set_vars` in `lib/confinement.nix` | `a claude-code session does not redirect TMPDIR, and /tmp is granted, so a tool falling back to it writes outside the project and nothing reports it`, and the same line for `opencode` and for `pi`. Planted after the close-out audit found this the one root no layer covered: the derived mirror above it reads `XDG_[A-Z_]+=` out of the shell hook and cannot see a name that is not an XDG one, and `check_j2_1` reads its root set out of the description, so a dropped root shrinks both sides of that comparison. Confirmed to be the reason for the literal rather than a duplicate of it: with the plant in place the rest of the component layer still passed | [x] |
...
| `require_gnu_find` | Make the probe fail, as it would on a `find` without `-printf` | `the manifests this suite compares need GNU find…`, and observed on both routes into it — `check_pi` through `dir_manifest` and `check_j8_1` through `tree_manifest` | [x] |
...
| `check_j8_1` | Neutralise the pointing while leaving the grant — this table's originally planned *remove the configuration key that points the agent at the granted roots*, in the form that turned out to exist — by writing the surface to `s.path "$PWD" + ".planted"` in `lib/confined-agent.nix`, one edit that disables both `skillSurface` kinds | six failures, two per agent: `the <agent> session never read the declared surface at … so it did not arrive`. And the discrimination the plant exists for: the grant arm **still passed**, so a session that is given the surface and never told about it is distinguishable from one that is told and not given it | [x] |
| `check_j8_1` | Widen the grant to read-write, `--read` → `--allow` on the wrapper's `nono run` argv — this table's originally planned *make the grant read-write instead of read-only* | bites three ways per agent, which is what makes the read-only half of the property real rather than assumed: `the <agent> session was granted  but the consumer declared …` (the grant arm reads the flags off the wrapper's own `execve` line, and `--read` is gone), `the grant the <agent> entry point passes let a session write to the declared surface (WROTE), so the surface is handed over rather than lent`, and the surface's own manifest changing digest afterwards. Watching this plant is also what showed the write arm had to parse `--read`, `--allow` and `--write` alike: built from the `--read` flags alone it degenerated under the plant into a run with no grant at all, denied for the wrong reason | [x] |
...
- **nono's remaining configuration surfaces** (R5's neighbourhood). All six are now measured — profiles, `NONO_PROFILE`, `NONO_ALLOW`, `--extends`, `config.toml` and `--bypass-protection` — and none widens a pinned description, which is [D19](#d19). What remains a gap is that `check_r5` asserts over the two channels a checkout controls, the in-project profile and the in-project `config.toml`, and not over the two that arrive at the invocation: `--extends` and `--bypass-protection` are flags the wrapper does not pass, and their absence is asserted by `check_r4`'s reading of the wrapper text rather than by a session. The other two channels an invocation carries are covered — `NONO_PROFILE` by `check_r5`'s first three arms and `NONO_ALLOW` by its fourth, which measures the widening adding exactly one grant and withdrawing none. A project-level `trust-policy.json` is uncovered on purpose: it selects which files are verified against which keys, so it belongs to trust enforcement, which this feature has no scenario for and does not enable.
...
- **The two platforms' effective reach compared.** SC-8's second clause. CI compares the two descriptions and that comparison is of authorship rather than of reach, because nono's own floor groups resolve differently per platform and neither runner can see the other's. Closing it means resolving both on their own machines and diffing the two manifests by hand, which needs one of each at once.

- **That the credential-capture exclusion would be caught if it regressed.** `M9e` moved `check_opencode`'s and `check_pi`'s capture files inside the granted project, because a confined child resolves its inherited descriptors' paths at startup and Seatbelt refuses them by path. Landlock does not re-check an already-open descriptor, so on Linux the regression is invisible by construction and no plant can be watched from here. Needs a Mac: put the capture back outside the granted set and the session fails to start.

- **A machine with no warm store.** Every timing and every substituter claim this feature makes was measured on a checkout whose `.cache` was already warm. What a stranger's first `nix develop` actually costs, and whether the declared substituter is reached at all, is unmeasured.

- **Anything the mechanism says on a terminal rather than on a stream.** Redirecting the three streams was taken to cover every question the mechanism can ask, and it does not: the working-directory consent reads `stdin` and is covered by that reasoning, while the save-profile review opens the terminal directly and reached a real run of the suite after the check that caused it had already reported `PASS`. Both are now unobservable for the same reason — a pty is needed and a nested sandbox denies `/dev/ptmx`, so a check would skip on exactly the machine most likely to run it. `NONO_NO_SAVE_PROMPT=1` in the entry point and in the two session environments is asserted by nothing.

- **Your own trust settings for the substituter.** The flake declares `https://cache.numtide.com` and its key, and passes `--accept-flake-config` everywhere. Whether an individual nix installation honours that depends on whether the invoking user is a trusted user, which is a property of the host's `nix.conf` and not of anything this repository can assert. A run that silently built from source would still be green.

- **No automated run fails for a static stage.** Stages 1, 2, 4 and 5 of the type-checking table above are now named in the `justfile` and run by one command, so they are no longer a human's memory of which files to pass to which tool — but they are still a human's memory of running them at all: `scripts/validate.sh` runs none of them and CI runs only the entry point, so an unformatted file, an unlinted script or an action pin that no longer matches its tag comment reaches a green run. Closing this means either the entry point refusing on a formatting difference or a second CI job, both of which change what a green run promises, so it belongs in a follow-up rather than at close-out.
