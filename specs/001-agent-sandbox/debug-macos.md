...
The per-platform `check_sc1` equalities do read the resolved grant, but each subtracts a floor derived by resolving a `{meta}`-only description — and the floor is exactly where the platform difference lives, since nono's `system_read_macos` and `system_read_linux_core` grant a different set of host paths, each wherever they exist on the host. `system_read_macos` was described here as granting `/private`; it does not, and the difference matters for anything looking for an ungranted scratch root. What it names is `/private/etc`, `/private/etc/ssl`, `/private/var/db/dyld`, `/var/db`, `/System/Library`, `/Library`, `/usr/share` and — read off `nono profile groups system_read_macos` — **`/tmp`, which it resolves to `/private/tmp`**. So `/private` is granted in parts rather than whole, and `/tmp` is granted outright. The one thing that differs between platforms is therefore subtracted on both sides of every equality. The host-dependence of that grant is already carried to `M10a`; its consequence for FR-20 and SC-8 is what is new.
