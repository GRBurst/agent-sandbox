...
- **`check_r9`'s `nohost` arm was not at risk of going vacuous — it already was**, and the thing that had defeated it was the entry point under test, whose `mkdir -p "$XDG_CONFIG_HOME"` runs before `nono run`. The fallback to `$HOME/.config` fires only when that directory does *not* exist, measured over three cases.
- **A missing `find -printf` was a silent pass, reproduced rather than argued**: with the guard removed and the manifest emptied, `check_pi` exits 0.
- **The cross-platform job cannot observe reach at all, and no filter fixes that** — the resolved manifest lists no non-store grant while `nono why` answers `granted_path` for `/usr/bin` against the same description. It is renamed for the claim it can make; SC-8's second clause is a hand step.
- **The `--accept-flake-config` gap was 30 sites across four files**, not 21 in one, and two of them were `pinned_bin` and `outside_root` in `validate.sh` — the helpers every integration check routes through.
