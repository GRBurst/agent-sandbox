...
### M9e — The eleven failures the macOS arm found (Status: IMPLEMENTING)
...
- [ ] The consequence for a user is stated in `docs/HANDBOOK.md`: a project that *is* the home directory cannot start an agent, because the probe has nothing outside the project left to be refused. That is the honest answer — confining a session whose project is the whole home grants it everything — and per **P9** it is a refusal that says so, not a session that starts unverified
...
- [ ] The gap is named in `docs/HANDBOOK.md`
...
- [ ] The banner plant is proven for `check_j8_1` — declaring the two skill directories' common parent fails both the arrival arm and *was not granted the declared surface at …/host/alpha*. **`check_r9`'s `nohost` plant is unrun**: three attempts were defeated by an intermittent `cannot open SQLite database … fetcher-cache-v4.sqlite` that leaves the check reporting its own anti-vacuity control instead
...
**Where a fresh session picks this up.** All five classes have landed. D and E went in as `d5d333d` then `dc5e938`, in that order and for the recorded reason: D moved the captures, E then narrowed the span that reads them. On this host the unit, component and integration layers are green — 6, 3 and 22 checks — with `check_j6_2`, `check_r11`, `check_opencode` and `check_pi` among them, so **run 8 is the measurement that matters** and nothing below should be started before it is read.

Two plants are owed to that run rather than to a host: D's, which Landlock cannot express, and E's literal Ubuntu case, which needs a `gpg` the profile grants. Both are stated in place above with what stands in for them here.

What is left after run 8 is the two boxes in class B — the handbook's coverage gap and `check_r9`'s unrun plant — and then *Closing the arm*, whose first box is a deletion. The evaluator's `fetcher-cache-v4.sqlite` failure is intermittent rather than a broken checkout: it recurred within a minute of a successful `nix eval`, and a check that hits it reports *the agent table has 0 entries*, which is its own anti-vacuity control firing on an unusable evaluator rather than the property failing. Retry rather than diagnose. The shell in this environment also fails heredocs with `can't create temp file for here document`, so a commit message goes to a file and `git commit -F` reads it back.
