...
Two things this leaves open, both for [M10a](#m10a--close-out-status-implementing) rather than for here. The last candidate writes under the real home, which is a write outside the project and so is the accepted-leak question the directive was posed to settle. And the derivation's answer is only as good as the list it tries: on a host offering none of the three, the suite refuses to run rather than reporting a boundary it could not observe — the right failure, but a failure.
...
| D | `check_opencode` `check_pi` `check_j6_2` | a confined child cannot read the capture files it was handed, because those now sit somewhere the session is granted nothing — `check_j6_2` joined in run 7 |
| E | `check_j6_2` | the traced span covers the demand arm, whose deliberate reach is counted against the plain commit — introduced by class B, found in run 7 |
...
Implementing them changed the table above in one place: **`check_r1` is not class A.** Its wording assertion was the smaller of two defects, and the larger one is that nono protects `$HOME/.ssh` of its own accord, so a key refused inside a session says nothing about the project boundary. It became a commit of its own, and what it measured is recorded in [research.md § M9e](research.md#m9e--the-four-classes-answered-without-a-mac).
Five commits have landed, each with its plants proven: `ec31fbe` (C), `66edce3` (`check_r1`), `937b979` (A), `61ffb2c` and `fb0333b` (B).

**Run 7 measured them.** macOS fell from eleven failures to three, Linux rose from none to one. Classes A, B and C are answered on both platforms. What was left was class D, which had never been started, and one regression class B introduced — recorded as class E below and in [debug-macos.md](debug-macos.md#what-run-7-left). Both have since landed, as `d5d333d` and `dc5e938`, and neither turned out to be a defect in the confinement: both were defects in what a check *measured*. Run 8 is where they are answered.
...
- [x] A candidate that cannot serve is skipped with its verdict — inside the project, or not readable from here — and only an exhausted list refuses, carrying every candidate's verdict so the reason it gave up is readable rather than guessed at (**P9**). With this the pre-flight writes **nothing** outside the project in any passing case, so no leak-registry entry and no `P1` accepted-leak entry is owed — the question [M10a](#m10a--close-out-status-implementing) carries for the harness is not the same question, and this one is gone rather than answered
...
- [x] The consequence for a user is stated in `docs/HANDBOOK.md`: a project that *is* the home directory cannot start an agent, because the probe has nothing outside the project left to be refused. That is the honest answer — confining a session whose project is the whole home grants it everything — and per **P9** it is a refusal that says so, not a session that starts unverified

**A — one denial, two spellings, and two assertions that should not exist**

- [x] `check_r2` **drops** the wording assertion, with a comment saying what carries the claim instead. It already has three controls — the same path written unconfined before the session, an in-project write inside the session, and a `granted` arm requiring the same probe to succeed — and those refute the "it may simply not have been there" alternative that the grep was standing in for
...
- [x] The derivation asserts its own control — the probe path exists and is readable unconfined — so an `ENOENT` phrase cannot be derived and make the assertion vacuous. This control was **added because its absence was measured**: without it, deleting the sample file made the phrase `No such file or directory` and the check failed downstream with a message naming the wrong cause
- [x] The refusal message a reader sees still names what was refused, not which errno was matched
- [x] Violations planted: `check_r2` given a target whose parent never existed, which fails on control 2 where that failure belongs; and `check_r8`'s sample file deleted, which fails on control 4. Both reverted

**`check_r1` — its own class, because the wording was the smaller half**

- [x] The session reads a **plain file beside the key** as well, denied by reach and by nothing else, and each read reports its own status so neither hides behind a shared exit code. nono protects `$HOME/.ssh` of its own accord — `jq '.filesystem.deny'` on the shipped description is `null` and nothing in `lib/` names `.ssh` — so a key refused inside a session says nothing about the project boundary: a session with no boundary at all would refuse it too
- [x] The positive-control arm grants that plain directory rather than the key's, because the platforms disagree about the credential and neither answer can be asserted as the other's. On Linux a grant naming `$HOME/.ssh` wins and the key reads out; on macOS the same grant is marked `[permanently restricted]` with a pointer to `filesystem.bypass_protection`. The shipped reach is identical on both, since neither description grants the directory
- [x] The arm also asserts the key stays out of reach when its neighbour is granted, which is what says the grant stayed as narrow as written
- [x] Violations planted: the shipped arm given the grant too, which fails both new reach assertions; and the granted arm pointed at `$HOME/.ssh` instead, which fails and demonstrates the Linux half of the divergence in situ. Both reverted
...
- [x] `commit_session`'s tracer requirement moves out of the shared helper and into `check_j6_2`. That alone recovers `check_r11` on darwin, with no new instrument and no platform split, and it is the smallest of the eleven fixes — **confirmed by run 7**, where `check_r11` passes on both platforms
- [x] `check_j6_2`'s denial-set assertion uses `strace` on Linux and the trailer on darwin behind one helper, with the platform split in one place. Both feeds are the same code in one binary — Landlock starves it and Seatbelt feeds it — so this is one property observed two ways
- [x] The helper subtracts a **baseline** taken from a no-op confined session rather than excluding a named path. Every darwin trailer carries `/Users/runner/.CFUserTextEncoding (read)` even when the boundary behaved perfectly, so *the denied set is empty* can never hold there; *the session added no denial of its own* holds on both platforms, spells no platform path, and reduces to an identity on Linux where the baseline is empty. **Run 7 showed the baseline is necessary but not sufficient**: it cancelled `.CFUserTextEncoding` as designed, and could not cancel the capture files, because a floor can only cancel a path it also produced and the floor's captures have different names. That remainder is class D, not a defect in the baseline
- [x] The helper checks the trailer against **its own header count** and fails when the two disagree, so completeness is asserted rather than assumed
- [x] The non-emptiness control becomes *nono reported at all* — either an `N paths blocked` header or `No path denials were observed during this session.`, both of which prove the supervisor was watching. `git log` is not the substitute: it proves the commit, not the observation. `session_reported` is that control, and it was measured refusing to read silence as evidence: a forced no-tracer run on Linux failed with *nono said nothing about what the session was refused*
- [x] Every assertion that reads grants out of argv reads the banner instead, on both platforms — through the **existing** `granted_reach` and `reach_grants`, so no parser was written. Their vocabulary is `{r, w, x, r+w, net, +}`, wider than the `{r, w, r+w}` this task wrote down, and the line is `<mode> <path> (dir)` after whitespace is squeezed, so no `(dir|file)` regex is needed. `check_j8_1` asserts per path rather than by whole-set comparison, because the capability set is everything the session was granted — the substrate and the project among it — while the check is about the surface
- [x] `check_r9`'s control that the wrapper reached the mechanism at all survives the change — the banner exists only if a session started, which is the same proof by another route. It replaced `grep -qE 'execve\("[^"]*/nono"'` with `[ -s "$grant" ]`, and `check_r9` now needs no tracer at all
- [x] Two classes of banner line are dropped where the grant is recorded, because both move between two runs of the same description: nono's `+ N system/group paths (-v to show)` summary, measured reading `34` against `33` for two sessions whose `-v` expansions were identical at 163 paths, and the `/proc/<pid>` lines, which differ on every run by construction. Reading the wider set is what exposed this; argv had hidden it
- [x] `check_j8_1`'s positive read arm **skips on darwin with the reason**. No instrument there observes a permitted read: `dtruss` needs SIP off, `fs_usage` needs root, the trailer records only denials, and the audit trail records only grants — mode-blind ones at that, so it serves no assertion here. `check_substrate_denials` is the precedent for the skip and for where the `uname` gate goes. The check reports `$SKIP_STATUS` rather than passing, so the strongest half of its property cannot read as asserted
  - [x] The gap is named in `docs/HANDBOOK.md`, under *Where the two platforms differ*, and named more precisely than the skip message manages: what darwin loses is arrival for `claude-code` and `pi` plus the undeclared-neighbour arm for all three, while the whole grant arm still runs there and `check_opencode` covers arrival for `opencode` by enumeration on both platforms
- [x] The trailer parser is falsifiable from either platform: `check_trailer_parse` is a **unit** check driven by five recorded stderr fixtures, because a Linux `nono run` prints no trailer at all — not even the `No path denials were observed` sentence an earlier round recorded — so the darwin branch cannot be exercised by starting a session on the platform that runs the suite unattended. Three plants bite: the marker left on the path, the count self-check disabled, and the recogniser made to match anything
- [x] The banner plant is proven for `check_j8_1` — declaring the two skill directories' common parent fails both the arrival arm and *was not granted the declared surface at …/host/alpha*. `check_r9`'s `nohost` plant is proven too: granting `$home/skills/gamma` to that arm alone fails the check for all three agents, each naming the line the deletion added — *deleting the host confinement description changed what the claude-code session was granted, so configuration outside the boundary decided the boundary: 132a133 > r+w …/home/skills/gamma (dir)*. What had defeated three earlier attempts was `nix` run from a shell that had not entered the environment, not an intermittent evaluator

**D — the captures a confined child cannot read**

Run 7 added a third check to this class. `check_j6_2`'s macOS trailer names `probe.err (read)` and `probe.out (read)`, which is the same position `paths.*` and `auth.*` are in — a capture outside the granted set, refused by path at startup. So the class is about where a capture lives, and the bun agents are the loudest case rather than the only one.

Landed as `d5d333d`.

- [x] `check_opencode` and `check_pi` capture into `$project/.capture`, excluded from the one listing that would otherwise go vacuous. The `landed` control is satisfied by the agents' own writes, since their XDG roots are relocated into the project — so the risk was vacuity, not breakage. Measured after the change: 23 entries for `check_opencode` and 4 for `check_pi`, none of them under the capture directory. The pipe alternative is rejected: Landlock does not re-check an open descriptor, so nothing about it can be measured from Linux
...
`61ffb2c` moved the tracer from the plain commit to the whole session so the strace span would match the span the darwin trailer observes. That pulled the **demand arm** inside it, and the demand arm exists in order to reach for signing material and fail. On ubuntu-latest its `gpg` starts — the profile grants `/usr/bin/gpg` on purpose, so `check_r11`'s refusal means something — and gpg's `setlocale` reads `/usr/share/locale/locale.alias`, which is not granted. The evidence is in [research.md § M9e](research.md#m9e--what-run-7-measured-and-the-one-regression-it-introduced).

Journey 6.2 scopes its claim to the plain commit: *nothing outside the session's reach was read in order to produce **it***. A denial the demand arm earned is not `check_j6_2`'s to report, and `check_r11` already asserts it from the arm's exit status without a trace.

Landed as `dc5e938`.

- [x] `check_j6_2` asserts over the **ordinary commit's span only**, not the whole session. Measured before the fix: the whole-session span carries the denial, the plain-commit span carries `[0 paths, from 170 traced lines]`
- [x] The two arms stop sharing one measured span: the demand arm moved to a session of its own. The phase-marker alternative was rejected because it cannot be expressed on darwin — a trailer covers a whole session and nothing smaller — and a split that one platform cannot make is a split that only Linux asserts. What the arms still share is what the control needs: the same profile, home and checkout
...
- [x] `scripts/validate.sh` passes on both platforms — `35 passed` on Linux, and 35 again locally on a NixOS host — and the cross-platform reach comparison ran for the first time and passed. What that job actually compares is weaker than it reads, and is recorded under [M10a](#m10a--close-out-status-implementing)
...
**What runs 8 and 9 settled.** All five classes landed, D and E as `d5d333d` then `dc5e938` in that order and for the recorded reason: D moved the captures, E then narrowed the span that reads them. Run 8 came back `35 passed` on Linux and `33 passed, 2 skipped` on macOS, with the `platforms` job running for the first time and passing. Both remaining class B boxes are closed and *Closing the arm* is done.

Run 9 is the one that closes it, because run 8 was still carrying the instrumentation. It ran the same tree with the probe step and the `dbg`/`dbg_watch_*` calls deleted and returned the same counts, which matters most for `check_j2_1` and `check_j3_1`: their watcher had *replaced* each check's own cleanup trap, so removing it had to restore the original rather than drop it. Run 9 also has the property no earlier run had — its logs are the suite's output and nothing else.

One plant is still owed to a runner rather than to a host: D's, which Landlock cannot express. E's literal Ubuntu case is proven by the CI run itself. Both are stated in place above with what stands in for them here.
...
**Four weaknesses the audit of the green runs found**, none of them introduced by these fixes and none visible as a failure, all carried to [M10a](#m10a--close-out-status-implementing):
...
**Two weaknesses this task found and did not fix**, both carried to [M10a](#m10a--close-out-status-implementing):
...
### M10a — Close out (Status: IMPLEMENTING)

**Where a fresh session starts.** This is the last task in the feature and the only one still open. Everything before it is `IMPLEMENTED` or `DONE`; the suite is green on both platforms in CI and on a NixOS host, so nothing below is blocked on a measurement that has not been taken. Read [spec.md](spec.md) for what is being closed out against, then this task's boxes. [debug-macos.md](debug-macos.md) is the debugging record and is finished — worth reading for *What the green runs left standing* and *Ruled out*, not for instructions.

The work divides in three, and mixing them is what makes it look larger than it is. Most of it is **documentation**: the handbook, the root `README.md`, the open questions in `spec.md`, and the drift list, which is retired entry by entry with anything still true *moved* rather than deleted. A smaller part is **six defects the audit found in the checks themselves** — the `landed` control, `find -printf`, the cross-platform comparison, that job's vacuity, `check_r9`'s fallback and `--accept-flake-config` — each of which is a real change to `scripts/`, each stated with what it must satisfy rather than how to fix it. The rest is **two contradictions to resolve**, `/usr/bin` against `flake.nix`'s sentence, and the harness's own outside against P1's leak list; both need a decision before an edit.
...
- [x] Root `README.md` written: component table taken from the code, one `flowchart LR` for structure, one `sequenceDiagram` per phase including **a refused case of its own** (AGENTS.md §6), checked by eye in both themes. Every table row names a path and says what the file at it does, read off the file rather than off the plan — the ten components, then the flake's four per-system outputs as a `sh` block with a comment per line, because a list of commands is neither structure nor time. The structure diagram is one `flowchart LR` with `$XDG_STATE_HOME/nono` as its single dashed node, which is the one thing outside the boundary. **Two refused cases, not one**, because the repository denies two different things on purpose and each denial is its own demonstration: the pre-flight refusing a host it cannot prove confinement on, and a session refused a read outside its project. Both-theme correctness holds by construction rather than by inspection — there is no `%%{init}%%`, no `fill:` and no `stroke:` colour anywhere, and the one `classDef` sets `stroke-dasharray` alone, so nothing overrides the reader's theme. Nothing in the environment renders mermaid, so this is the strongest check available. The handbook's drift entry for the missing `README.md` is deleted with the same commit, which is the box above's third survivor retiring

- [x] The migration path for a consumer with a host-global setup is documented (FR-21) — which half of their setup comes with them and which does not, how to declare the authoring surface once for the machine (FR-25), that the prior-art arrangement granted the authenticating agent's state read-write and that [D14](plan.md#decisions) replaces it, so a migrating consumer knows what they are giving up and what they get back. Landed as *Migrating a setup you already have* in `docs/HANDBOOK.md`. The two halves were already stated in pieces — the skills route in *Bringing your own skills in*, the credential in *The API key inside a session is not your API key* — and this section is where a migrating consumer reads the **line** rather than assembling it, which is what Risk 400 asks for: a consumer whose skill loaded but whose stored session did not would otherwise conclude the boundary is arbitrary. What was documented nowhere is [D14](plan.md#d14)'s trade, and it is the section's last paragraph: the arrangement this replaces shared one credential between agents *by file*, which meant the authenticating agent's credential directory granted **read-write** inside the boundary. That is what is given up. What is got back is the thing it was for — authenticate once, every agent, every project — without the writable grant, plus two properties the old route could not offer: no agent reads another's store, and a value copied out is worth nothing because the provider has never seen it. FR-25's declaration is linked rather than restated, per FR-12

- [x] SC-9: for every agent, either the surface reaches every location that agent reads extensions from, or the location it does not reach is **named**. A consumer must not have to find out by experiment which of their own extensions came with them. **The handbook's prose was not complete, and SC-9 asks for completeness.** *Skills are the whole of it* named six uncovered locations for `claude`, three for `pi` and three for `opencode`; the enumeration [M8e measured](research.md#m8e--where-each-agent-reads-its-declarative-extensions-from) has more than that — `opencode`'s `prompts/` and `themes/`, its `$HOME`-relative `~/.agents/skills` and `~/.claude/skills`, `pi`'s `~/.agents/skills`, `claude`'s ancestor walk for `CLAUDE.md`, `.claude/rules` and `.mcp.json` up to `/`, and `/etc/claude-code/**`. Three per-agent tables now carry every row of that enumeration with a yes or a no against each. What the prose could not express and the tables make plain is that there are **two kinds of no**: a relocation, where the agent looks in its new root and the host copy is simply elsewhere, and a location no variable moves — the `$HOME`-relative rows and the ancestor walk — where the agent still looks outside the project and is *denied*. The second kind is the set `check_j8_2` asserts over, so the tables say which rows carry that weight instead of leaving a reader to infer that every no is the same no
...
- [x] The way to run an agent unconfined — by not invoking the confined entry point — is described rather than concealed (FR-10). *Starting an agent* already said the unconfined binary is reachable only by store path, which **mentions** the route without describing it, and FR-10 asks for the second. It now carries the two commands that do it, run here rather than composed from memory: `grep -o '/nix/store/[^ ]*/bin/claude' "$(command -v claude)"` reads the path out of the wrapper, and executing that path is the whole of it. Beside them, the four things switched off in one step — the project boundary, the credential substitution and with it your *real* key rather than a substitute, every relocation, and the git identity along with the host `core.hooksPath` and `credential.helper` the wrapper was suppressing. The framing matters as much as the commands: a session unconfined because someone typed a store path is a decision, while one unconfined because a check failed quietly would be a defect, and FR-10's `77` exists to keep the second impossible. There is no partial version, because the boundary is established once before the agent starts

- [x] Every open question in `spec.md` resolved in place with a one-line outcome, including the two assumptions the plan was still to confirm. **There are nineteen risks, not eighteen**, and all nineteen now carry an `*Outcome*` line stating what happened rather than what was planned; each was traced to the check that covers it or to the coverage gap that does not, so none is a summary of its own mitigation. Written in the spec's own vocabulary — Journey *n*, R*n*, SC*n* — rather than by check function name, because this file's review checklist already flags the implementation detail it carries and nineteen new function names would have made that materially worse. Of the assumptions, only one was still outstanding and it resolved as **confirmed unnecessary rather than confirmed true**: nothing is at rest to be reachable on either platform, so Journey 4's observable is an absence and is platform-independent by construction. Four outcomes are worth reading on their own: risk 1's premise was *falsified*, so FR-7's across-agents axis was replaced rather than met; risk 5's reach never diverged because the description carries no deny rule for a platform to interpret; risk 9 realised **twice inside this feature itself**, the pre-flight having written a canary into the home it was meant to prove unreachable; and risk 14 is the only one that closed as an **open gap** — no check exercises a streamed response, because none can make a live provider call at all
...
- [x] **`check_r9`'s `nohost` arm rests on an undocumented fallback.** It plants the host description at `$home/.config/nono/profiles/host.json` while the session runs with `XDG_CONFIG_HOME=$outside/cfg`, and nothing creates that directory — so nono falls back to the host's `$HOME/.config`, which is why the arm reads the planted file at all. Either the arm states the dependency or it points `XDG_CONFIG_HOME` where it means to, because anything that creates `$outside/cfg` makes it vacuous in silence. **Took the second, and the audit's diagnosis was wrong in a way that mattered: the arm was not at risk of going vacuous, it already was.** The fallback is real but *conditional*, measured over three cases with uniquely-named profiles — a plant under `$XDG_CONFIG_HOME` is found; a plant under `$HOME/.config` is found only when `$XDG_CONFIG_HOME` does **not exist**; and with `$XDG_CONFIG_HOME` existing and empty, the `$HOME/.config` plant is **not** found. So it is a fallback and not a second search path. And `lib/confined-agent.nix` runs `mkdir -p "$XDG_CONFIG_HOME"` before it reaches `nono run` — so the third case is the one that applied, the planted host description sat outside nono's search path, and deleting it could not have changed anything. The thing that created `$outside/cfg` was the entry point under test. `XDG_CONFIG_HOME` now names `$home/.config`, where the plant is: `nono profile list` lists it, so the arm observes a description nono **can** see and ignores anyway — which is R9's actual claim, since the entry point names its own profile by store path and an argument beats every discovery route. One earlier reading was a false positive worth recording: grepping `nono profile list` for `host` matches the built-in `linux-host-compat`'s description text, *"…for host runtime…"*, not a profile of that name. `check_j8_1` carries the same uncreated-`$outside/cfg` pattern and was deliberately left alone — it plants nothing in the fallback location, so the vacuity does not arise there

- [x] **`check_j1_1` spends a shared quota.** Resolving `github:GRBurst/agent-sandbox` goes through the GitHub API, sixty unauthenticated calls an hour per address, so a few suite runs in one hour turn the end-to-end check red with `HTTP error 403` on a tree that is fine. Either the failure names the quota so a reader is not misled, or the ref resolves without the API — and if it is pinned to a revision, FR-19's claim that the canonical ref is consumable needs another home, because a pinned check no longer makes it. **Took the first, and the box's own parenthesis is why**: pinning to a revision resolves without the API but stops making FR-19's claim, which is that the canonical reference — `HEAD` on it — is consumable, so the claim would need rehousing and the check would be weaker. The quota is the cheaper thing to live with once it is *legible*. The failure path now recognises `HTTP error 403` or `rate limit exceeded` in what nix printed and says the quota is spent and that this says nothing about whether the reference is consumable. **Still a failure, not a skip**, deliberately: a skip would let the only check that consumes this repository the way a stranger does go quiet exactly when a shared address is busiest. This was not hypothetical — the baseline run for this task hit it, which is how the recorded body came to be exact. Two plants in [plan.md](plan.md#planted-violations), and the second is the one that matters: a non-quota failure must keep the generic message, because a branch claiming the quota for every failure would explain away a genuinely broken end-to-end path and be worse than what it replaced. With the quota reset, `check_j1_1` was then run for real and passes

- [x] **The integration checks ignore the declared substituter.** `e2e.sh` passes `--accept-flake-config` at thirteen call sites and the workflow at two; none of the twenty-one `nix eval` and `nix build` invocations in `integration.sh` do, so on any runner whose user is not trusted — macos-latest, and every consumer's machine — those builds silently fall back to `cache.nixos.org` or to source. Measured as forty-four warnings in run 9's darwin log against zero on Linux. This is the same fact as the stranger's copyable command two boxes above, met from the other side; whichever way it is fixed, the two must agree. **The audit undercounted, and the count is the finding.** It named twenty-one sites in `integration.sh`; the flag was missing from **every** `nix` invocation naming a flake reference in the whole suite — 18 in `integration.sh`, 6 in `component.sh`, 4 in `unit.sh` and 2 in `scripts/validate.sh` itself, the last two being `pinned_bin` and `outside_root`, which every integration check routes through. Fixing only the file the box named would have left the two helpers that all of them call still falling back. All 30 now carry it, so the suite and the handbook's copyable command agree. **Eight invocations are deliberately exempt and the reason is written down** in `validate.sh`'s preamble, because the next reader would otherwise either add the flag or wonder: six are `nix eval --impure --expr` on a builtin, one is `nix eval --file flake.nix` reading the file as a plain expression, and one is a `fail` message rather than an invocation. None reads a flake, so none has a `nixConfig` to accept — measured at **zero** warnings. The untrusted case itself cannot be reproduced on this host, whose `trusted-users` includes the developing user, so the forty-four-warning figure stands on run 9's darwin log; what is reproduced locally is that the flag removes the per-invocation settings chatter, two lines to none

- [x] `research.md` consolidated to what is still true, per AGENTS.md §1 — the decisions and the criteria kept, the record of how each was checked dropped; the `codex` findings kept, since they are the follow-up feature's head start. **The box's premise turned out to be largely false for this file, and that is the finding.** Every one of its 219 subsections was classified before anything was cut: **208 carry a measured value, a decision, a criterion or a `codex` reference**, and only 6 were pure process narrative. There is no large "record of how each was checked" to drop, because each round was written up as findings rather than as a diary — so a one-third reduction was not available without deleting the evidence that `tasks.md` and `plan.md` cite. What was dropped: the six narrative sections, two of them generic `jq`/`bash` language bugs and four duplicating a trap recorded elsewhere; the five-run CI table, which [debug-macos.md](debug-macos.md#where-the-arm-stands) owns and which now keeps only the one cause that was never this repository's, the nix 2.22.1 darwin installer taking UID 301 that macOS reserves; and the narrative wrapper around six salvaged facts. Four `Method notes` headings were **retitled** rather than removed, because they described durable instrument behaviour and the heading was the only misleading part. 3176 → 3110 lines. Two properties were verified rather than assumed: all **36** anchors other files aim into this file still resolve — one of which was already broken before this work and is fixed in its own commit — and all 11 `codex` mentions survive. What a fuller consolidation would take is named rather than done: the harness traps are scattered across nine sections and would collapse into one, which is a restructuring rather than a deletion
