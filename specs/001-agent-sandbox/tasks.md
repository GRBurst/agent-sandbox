...
**Run 7 measured them.** macOS fell from eleven failures to three, Linux rose from none to one. Classes A, B and C are answered on both platforms. What is left is class D, which was never started, and one regression class B introduced — recorded as class E below and in [debug-macos.md](debug-macos.md#what-run-7-left).
...
- [ ] `commit_session`'s `probe.out` and `probe.err` move into the project on the same reasoning. It carries no `landed` control, so it needs no exclusion — but the noise floor's own capture must move with it, or the floor keeps producing paths the measured session cannot match
...
- [ ] `check_j6_2` asserts over the **plain commit's span only**, not the whole session. Measured before the fix: the whole-session span carries the denial, the plain-commit span carries `[0 paths, from 170 traced lines]`
- [ ] The two arms stop sharing one measured span. Either the demand arm moves to its own session, or the span is delimited within the session and each check reads its own — the second is what the phase-marker technique in [debug-macos.md](debug-macos.md#running-and-attributing-one-check) already demonstrates
- [ ] On darwin the trailer still cannot be narrowed, so `check_j6_2` there asserts over a session that contains **only** the plain commit. Making the Linux span match darwin's is what caused this; matching them by narrowing *both* is the direction that preserves the claim
- [ ] `ARMS_NOISE` is assigned only on the branch that creates the file, or the floor is produced on both. Today it names a file Linux never writes, which promises an observation that was not made
- [ ] Violation planted: the demand arm's grant restored to the measured span on a host where `gpg` resolves to a granted path, seen to reproduce `/usr/share/locale/locale.alias`. A NixOS host **cannot** plant this — its `gpg` is `path_not_granted`, so the `execve` fails and no locale read happens — so the plant needs the CI runner or a `PATH` pointed at a granted gpg
- [ ] Whether a session should inherit a `PATH` that reaches host binaries at all is settled, or recorded as known drift. `session_env` sets `TMPDIR`, `XDG_STATE_HOME` and `NONO_NO_UPDATE_CHECK` and not `PATH`, which is why this check's verdict varied by machine
...
**Where a fresh session picks this up.** Two classes are open, D and E, and they touch the same helper, so do **D first and E second**: D moves `commit_session`'s captures into the project, and E then narrows the span that reads them. Doing E first means measuring a span whose captures are still producing denials on darwin. Neither is open on evidence — D's design is settled to a `find` invocation, E's to the phase markers already recorded — and E carries the one plant that cannot be proven from a NixOS host. Then the two boxes in class B — the handbook's coverage gap and `check_r9`'s unrun plant — and then *Closing the arm*, whose first box is a deletion. The evaluator's `fetcher-cache-v4.sqlite` failure is intermittent rather than a broken checkout: it recurred within a minute of a successful `nix eval`, and a check that hits it reports *the agent table has 0 entries*, which is its own anti-vacuity control firing on an unusable evaluator rather than the property failing. Retry rather than diagnose. The shell in this environment also fails heredocs with `can't create temp file for here document`, so a commit message goes to a file and `git commit -F` reads it back.
